...
//Yacc Declarations
%token T_VAR T_DEF T_KEY T_ID T_NUM T_LBR T_RBR T_STR T_SHA T_LCG T_LOP T_OP1 T_OP2 T_OP3 T_OP4 T_IF T_ELSE T_IN T_LET T_CONSOLE T_DOCUMENT T_FOR T_DO T_WHILE
...
statement: declare terminator| expression terminator| for | if | do | '{'{scope[stop++]=sid++;} seqOfStmts '}' {stop--;} | T_CONSOLE '(' T_STR ')' | T_CONSOLE '(' list ')'|  terminator | T_DOCUMENT '(' T_STR ')' | T_DOCUMENT '(' list ')';
...
for: T_FOR '(' list ';' list ';' list ')' statement;
